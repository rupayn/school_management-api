...
export const addSchoolController=asyncHandler(async (req:express.Request, res:express.Response) => {
    const parsed=addSchoolSchema.safeParse(req.body)
    if(!parsed.success){
        throw new CustomError(400,parsed.error.errors[0].message,[parsed.error.format()])
    }
    ...
    if(existingSchool){
        throw new CustomError(400,"School already exists at this location")
    }
    ...
    return res.status(200).json(
        new CustomSuccessResponse(200,createdSchool,"School added successfully")
    )
});